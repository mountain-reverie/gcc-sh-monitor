...
static void __attribute__((always_inline)) inline fdct16_1d(float *dst, const float *src, int dst_stridea, int dst_strideb, int src_stridea, int src_strideb) {
  int i;
  for (i = 0;
       i < 16;
       i++) {
...
static __attribute__((always_inline)) inline void filter_freq_16(const float *src, int src_linesize, float *dst, int dst_linesize, AVExpr *expr, double *var_values, int sigma_th) {
  unsigned i;
...
void filter_freq_expr_16(DCTdnoizContext *s, const float *src, int src_linesize, float *dst, int dst_linesize, int thread_id) ;

void filter_freq_expr_16(DCTdnoizContext *s, const float *src, int src_linesize, float *dst, int dst_linesize, int thread_id) {
  filter_freq_16(src, src_linesize, dst, dst_linesize, s->expr[thread_id], s->var_values[thread_id], 0);
}
